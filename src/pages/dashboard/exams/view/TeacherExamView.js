...
export default function TeacherExamView() {
  ...
  console.log("📌 userId داخل TeacherExamView:", userId);
  ...
  const fetchExamData = async (examId, subject) => {
    console.log("📌 examId قبل الإرسال إلى API:", examId);
    console.log("📌 subject قبل الإرسال إلى API:", subject);
    try {
      setLoading(true);
      console.log("📡 جلب بيانات الامتحان ID:", examId, "📚 المادة:", subject);

      const response = await fetchExamQuestions(examId, subject, "teacher");
      console.log("📡 استدعاء API بـ examId:", examId);
      console.log("📡 استدعاء API بـ subject:", subject);
      console.log("📡 استدعاء API بـ examType: teacher");
  ...
  useEffect(() => {
    ...
    console.log("📡 البيانات المستقبلة من `router.query`:", router.query);
    console.log("📌 examId المستلم:", router.query.examId);
    console.log("📌 subject المستلم:", router.query.subject);
  ...
  const handleSubmitAnswer = async () => {
    ...
    try {
      // ✅ **جلب نوع الامتحان وتمريره**
      let examType = exam?.examType;
      if (!examType) {
        console.warn("⚠️ `examType` غير متوفر، سيتم تعيينه إلى 'unknown'.");
        examType = "unknown";
      }
      ...
      let correctAnswer = currentQuestion.correctAnswer
        ? currentQuestion.correctAnswer
            .replace(/^الإجابة الصحيحة هي:\s*/, "")
            .trim()
        : "❌ لم يتم العثور على إجابة صحيحة.";
      console.log("🔵 الإجابة الصحيحة بعد التنظيف:", correctAnswer);
      ...
      // ✅ **حساب الدرجة لكل إجابة**
      const score = isCorrect ? 1 : 0; // ✅ يتم حساب الدرجة تلقائيًا
      const totalQuestions = exam.questions.length; // ✅ يتم جلب عدد الأسئلة

      console.log("🎯 الدرجة المحتسبة:", score);
      console.log("📊 إجمالي عدد الأسئلة:", totalQuestions);
      ...
      const result = await submitStudentAnswer(
        ...
        currentQuestion._id,
        selectedAnswer,
        examType, // ✅ إرسال `examType`
        score, // ✅ تمرير `score` كعدد صحيح
        totalQuestions
      );
  ...
  const handleFinishExam = async () => {
    try {
      ...
      console.log("🔍 التحقق من البيانات قبل الإرسال...");
      console.log("🆔 examId:", examId);
      console.log("👤 userId:", userId);
      ...
      const payload = {
        ...
        isFinalSubmission: true, // ✅ **إضافة هذا الحقل**
      };
  ...
  return (
    <div className="min-h-screen bg-gray-100" dir="rtl">
      ...
      <div className="flex flex-col items-center justify-center p-6 space-y-4">
        ...
        <Head>
          <title>امتحانات توجيهي - معلمين | منصة الشبح</title>
          <meta
            name="description"
        ...
        {loading ? (
          ...
          <div className="w-full max-w-2xl p-4 bg-white rounded-lg shadow-md">
            ...
            >
              توليد شرح وحل مفصل 🧠 بالذكاء الاصطناعي{" "}
            </button>
