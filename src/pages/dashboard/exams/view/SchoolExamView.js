...
export default function SchoolExamView() {
  const { userId } = useAuth(); // ✅ استرداد userId مباشرة من AuthContext
  console.log("📌 userId داخل SchoolExamView:", userId);
  ...
  const fetchExamData = async (examId, subject) => {
    try {
      ...
      const response = await fetchExamQuestions(examId, subject, "school");
  ...
  const handleSubmitAnswer = async () => {
    ...
    try {
      ...
      // ✅ **تحديث الإجابة الصحيحة بعد استلامها من السيرفر**
      correctAnswer = result.correctAnswer
        ? result.correctAnswer.trim()
        : "❌ لم يتم العثور على إجابة صحيحة.";

      console.log("🔵 الإجابة الصحيحة المسترجعة بعد تحديثها:", correctAnswer);
  ...
  return (
    <div className="min-h-screen bg-gray-100" dir="rtl">
      ...
      <div className="flex flex-col items-center justify-center p-6 space-y-4">
        ...
        <Head>
          <title>امتحانات توجيهي - مدارس | منصة الشبح</title>
          <meta
            name="description"
