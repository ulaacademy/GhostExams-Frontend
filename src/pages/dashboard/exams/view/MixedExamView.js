...
export default function MixedExamView() {
  const { userId } = useAuth(); // ✅ استرداد userId مباشرة من AuthContext
  console.log("📌 userId داخل MixedExamView:", userId);
  ...
  useEffect(() => {
    ...
    if (examId) {
      fetchMixedExamQuestions(examId, userId)
        .then((data) => {
          console.log("✅ بيانات الامتحان المختلط المسترجعة:", data);
        ...
        .catch((error) =>
          console.error("❌ خطأ في جلب الأسئلة المختلطة:", error)
        );
    }
  }, [examId, userId]);
  ...
  // ✅ دالة جلب بيانات الامتحان المختلط

  // ✅ دالة جلب بيانات الامتحان المختلط مع useCallback
  const fetchMixedExam = useCallback(
    async (examId, subject) => {
      try {
        setLoading(true);
        console.log(
          "📡 جلب بيانات الامتحان المختلط ID:",
          examId,
          "📚 المادة:",
          subject
        );

        const response = await fetchMixedExamQuestions(examId, userId);
        ...
        setExam(response);
        setExamReady(true);
        console.log("✅ بيانات الامتحان المختلط المسترجعة:", response);
      } catch (error) {
        console.error("❌ خطأ أثناء جلب الامتحان المختلط:", error);
    ...
    [userId]
  ); // ✅ تحديد التبعيات لمنع إعادة إنشاء الدالة مع كل إعادة رندر

  // ✅ التحقق من router.query قبل استدعاء fetchMixedExam
  useEffect(() => {
    ...
    console.log("📡 استدعاء fetchMixedExam مع:", examId, subject);
    fetchMixedExam(examId, subject);
  }, [router.isReady, router.query, fetchMixedExam]); // ✅ إضافة `fetchMixedExam` إلى التبعيات
  ...
  return (
    <div className="min-h-screen bg-gray-100" dir="rtl">
      ...
      <div className="flex flex-col items-center justify-center p-6 space-y-4">
        ...
        <Head>
  <title>امتحانات توجيهي - ذكاء اصطناعي | منصة الشبح</title>
  <meta name="description" content="حل امتحانات المعلمين من سنوات سابقة مع التصحيح والتحليل الفوري. منصة الشبح للتوجيهي الأردني." />
  <meta name="keywords" content="امتحانات معلمين, توجيهي 2007, توجيهي 2008, اسئلة سنوات سابقة, امتحان عربي وزاري, امتحان رياضيات علمي" />
</Head>
        ...
        {loading ? (
          ...
          <div className="w-full max-w-2xl p-4 bg-white rounded-lg shadow-md">
            ...
            {exam?.questions?.[currentQuestionIndex]?.options?.length > 0 ? (
