...
export default function MixedTerms() {
  ...
  const handleSubjectClick = async (subjectId) => {
    ...
    try {
      ...
      const response = await axios.post(
        "https://ge-api.ghostexams.com/api/exams/generate-mixed-exam",
        {
          grade,
          term: selectedTerm,
          subject: subjectId,
      ...
      if (response.data.exam && response.data.exam._id) {
        ...
        router.push(
          `/dashboard/exams/view/MixedExamView?examId=${response.data.exam._id}&grade=${grade}&term=${selectedTerm}&subject=${subjectId}`
        );
      } else {
