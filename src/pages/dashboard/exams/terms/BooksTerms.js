...
export default function BooksTerms() {
  ...
  const handleSubjectClick = async (subjectId) => {
    ...
    try {
      ...
      const response = await axios.post(
        "https://ge-api.ghostexams.com/api/exams/generate-books-exam",
        {
          grade,
          term: selectedTerm,
          subject: subjectId,
      ...
      if (response.data.exam && response.data.exam._id) {
        ...
        router.push(
          `/dashboard/exams/view/BooksExamView?examId=${response.data.exam._id}&grade=${grade}&term=${selectedTerm}&subject=${subjectId}`
        );
      } else {
