...
export default function ArabicTerm1({ exams, usedFallback }) {
  const router = useRouter();
  const { user, token } = useAuth();

  const siteUrl = "https://ghostexams.com";
  const canonicalUrl = `${siteUrl}/tawjihi-2009/arabic/term-1`;

  const title = "امتحانات عربي توجيهي 2009 | الفصل الأول - GhostExams";
  const description =
    "قائمة امتحانات اللغة العربية (الفصل الأول) لتوجيهي 2009 في الأردن. اختر الامتحان حسب الوحدة وابدأ التجربة.";

  const ogImage = `${siteUrl}/og/arabic-term-1-2009.jpg`; // حط صورة فعلية داخل public/og

  // ✅ نص تعريفي يساعد السيو (محتوى نصي قابل للفهرسة)
  const introText = `هذه صفحة امتحانات اللغة العربية لتوجيهي الأردن 2009 (الفصل الأول) على GhostExams.
...
ابدأ بالتجربة، وراقب مستواك تدريجيًا من خلال نمط أسئلة قريب من النمط الوزاري.`;
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "اللغة العربية", item: `${siteUrl}/tawjihi-2009/arabic` },
      { "@type": "ListItem", position: 3, name: "الفصل الأول", item: canonicalUrl },
    ],
  };
  ...
  const itemListJsonLd = {
    "@context": "https://schema.org",
    "@type": "ItemList",
    name: "امتحانات عربي توجيهي 2009 - الفصل الأول",
    itemListOrder: "https://schema.org/ItemListUnordered",
    numberOfItems: exams?.length || 0,
    itemListElement: (exams || []).map((exam, idx) => {
      const previewUrl = `${siteUrl}/tawjihi-2009/arabic/term-1/${exam._id}`;
      return {
        "@type": "ListItem",
        position: idx + 1,
  ...
  // ✅ حماية زر بدء الامتحان الحقيقي (لو احتجته لاحقًا)
      ...
      <main className="pt-24 pb-16 px-4 sm:px-6 max-w-6xl mx-auto" dir="rtl">
        ...
        <header className="text-center px-1 sm:px-0">
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-yellow-400 leading-snug sm:leading-tight">
            امتحانات عربي توجيهي 2009 — الفصل الأول
          </h1>
        ...
        {!exams || exams.length === 0 ? (
          <div className="mt-8 bg-gray-800/60 border border-yellow-500/15 rounded-2xl p-6 text-gray-300 text-center">
            لا توجد امتحانات حالياً للفصل الأول (عربي).
          </div>
        ) : (
          <section className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {exams.map((exam) => {
              const previewHref = `/tawjihi-2009/arabic/term-1/${exam._id}`;

              // ✅ نص صغير لكل كرت (SEO friendly) — يستفيد من الاسم + البيانات
              const seoCardText = `امتحان عربي توجيهي 2009 (الفصل الأول) — ${
              ...
              return (
                ...
                >
                  ...
                  {/* ✅ (اختياري) عرض 2-3 أسئلة لو صار موجود sampleQuestions من الـ API */}
