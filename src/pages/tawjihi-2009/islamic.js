...
export default function Tawjihi2009Islamic() {
  const router = useRouter();

  const siteUrl = "https://ghostexams.com";
  const canonicalUrl = `${siteUrl}/tawjihi-2009/islamic`;
  ...
  // ✅ OG Image: public/og/islamic-2009.jpg
  const ogImage = `${siteUrl}/og/islamic-2009.jpg`;

  const introText = `منصة GhostExams توفر امتحانات التربية الإسلامية لتوجيهي الأردن 2009 بصيغة إلكترونية، مع بنك أسئلة منظم يساعدك على مراجعة المادة بطريقة واضحة وسريعة.
ستجد هنا روابط الامتحانات مرتبة حسب الفصل الأول والفصل الثاني (وقريبًا حسب الوحدات أيضًا)، لتختار المسار المناسب وتتابع التدريب بشكل تدريجي.
الامتحانات مصممة لتقيس فهمك وتدربك على شكل السؤال والخيارات المتوقعة وفق النمط الوزاري، وتزيد جاهزيتك قبل الامتحان النهائي.`;
  ...
  const webPageJsonLd = {
    "@context": "https://schema.org",
    "@type": "WebPage",
    name: "امتحانات تربية إسلامية توجيهي 2009",
    url: canonicalUrl,
    inLanguage: "ar-JO",
    description,
      ...
      <main className="pt-24 pb-16 px-4 sm:px-6 max-w-6xl mx-auto" dir="rtl">
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mt-8">
          <Link
            ...
            aria-label="الانتقال إلى امتحانات الفصل الأول تربية إسلامية توجيهي 2009"
          >
          ...
          <Link
            href={term2Url}
            className="group bg-gray-800/70 hover:bg-gray-800 border border-yellow-500/15 hover:border-yellow-500/40 rounded-2xl p-5 sm:p-6 transition shadow-lg min-h-[180px] flex flex-col"
            aria-label="الانتقال إلى امتحانات الفصل الثاني تربية إسلامية توجيهي 2009"
          >
            <div className="flex items-start justify-between gap-3 mb-3">
              <h2 className="text-lg sm:text-xl font-extrabold text-yellow-300">
                الفصل الثاني
