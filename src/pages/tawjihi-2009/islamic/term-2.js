...
export default function IslamicTerm2({ exams, usedFallback }) {
  ...
  const title = "امتحانات دين توجيهي 2009 | الفصل الثاني - GhostExams";
  const description =
    "قائمة امتحانات التربية الإسلامية (الفصل الثاني) لتوجيهي 2009 في الأردن. استعرض الامتحانات حسب الوحدة مع مدة الامتحان وعدد الأسئلة.";
  ...
  const ogImage = `${siteUrl}/og/islamic-term-2-2009.jpg`;

  const introText = `هذه صفحة امتحانات التربية الإسلامية لتوجيهي الأردن 2009 (الفصل الثاني) على GhostExams.
...
اضغط على "عرض الامتحان" لمعاينة بيانات الامتحان (صفحة SEO) قبل الدخول للامتحان من لوحة الطالب.`;
  ...
  const itemListJsonLd = {
    "@context": "https://schema.org",
    "@type": "ItemList",
    name: "امتحانات دين توجيهي 2009 - الفصل الثاني",
    ...
    itemListElement: (exams || []).map((exam, idx) => ({
      ...
      url: `${siteUrl}/tawjihi-2009/islamic/term-2/${exam._id}`,
    })),
  };
      ...
      <main className="pt-24 pb-16 px-4 sm:px-6 max-w-6xl mx-auto" dir="rtl">
        ...
        <header className="text-center px-1 sm:px-0">
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-yellow-400 leading-snug sm:leading-tight">
            امتحانات دين توجيهي 2009 — الفصل الثاني
          </h1>
        ...
        {!exams || exams.length === 0 ? (
          <div className="mt-8 bg-gray-800/60 border border-yellow-500/15 rounded-2xl p-6 text-gray-300 text-center">
            لا توجد امتحانات حالياً للفصل الثاني (دين).
          </div>
        ) : (
          <section className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {exams.map((exam) => {
              const previewHref = `/tawjihi-2009/islamic/term-2/${exam._id}`;
              const seoCardText = `امتحان دين توجيهي 2009 (الفصل الثاني) — ${
