...
export default function IslamicTerm1({ exams, usedFallback }) {
  const router = useRouter();
  useAuth();

  const siteUrl = "https://ghostexams.com";
  const canonicalUrl = `${siteUrl}/tawjihi-2009/islamic/term-1`;

  const title = "امتحانات دين توجيهي 2009 | الفصل الأول - GhostExams";
  const description =
    "قائمة امتحانات التربية الإسلامية (الفصل الأول) لتوجيهي 2009 في الأردن. استعرض الامتحانات حسب الوحدة مع مدة الامتحان وعدد الأسئلة.";

  const keywords =
    "امتحانات دين توجيهي 2009, امتحانات تربية اسلامية توجيهي 2009, بنك اسئلة دين توجيهي 2009, بنك اسئلة تربية اسلامية, امتحانات الكترونية دين, نمط وزاري دين, توجيهي الاردن 2009 دين";

  // OG Image (ضع صورة فعلية داخل public/og)
  const ogImage = `${siteUrl}/og/islamic-term-1-2009.jpg`;

  const introText = `هذه صفحة امتحانات التربية الإسلامية لتوجيهي الأردن 2009 (الفصل الأول) على GhostExams.
ستجد هنا الامتحانات الحقيقية الموجودة بالموقع مرتبة حسب اسم الوحدة/الكتاب، مع مدة الامتحان وعدد الأسئلة.
اضغط على "عرض الامتحان" لمعاينة بيانات الامتحان (صفحة SEO) قبل الدخول للامتحان من لوحة الطالب.`;
  ...
  const itemListJsonLd = {
    "@context": "https://schema.org",
    "@type": "ItemList",
    name: "امتحانات دين توجيهي 2009 - الفصل الأول",
    itemListOrder: "https://schema.org/ItemListUnordered",
    numberOfItems: exams?.length || 0,
    itemListElement: (exams || []).map((exam, idx) => ({
      "@type": "ListItem",
      position: idx + 1,
      name: exam.examName || `امتحان دين ${idx + 1}`,
      url: `${siteUrl}/tawjihi-2009/islamic/term-1/${exam._id}`,
    })),
  };
      ...
      <main className="pt-24 pb-16 px-4 sm:px-6 max-w-6xl mx-auto" dir="rtl">
        ...
        <header className="text-center px-1 sm:px-0">
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-yellow-400 leading-snug sm:leading-tight">
            امتحانات دين توجيهي 2009 — الفصل الأول
          </h1>
        ...
        {!exams || exams.length === 0 ? (
          <div className="mt-8 bg-gray-800/60 border border-yellow-500/15 rounded-2xl p-6 text-gray-300 text-center">
            لا توجد امتحانات حالياً للفصل الأول (دين).
          </div>
        ) : (
          <section className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {exams.map((exam) => {
              const previewHref = `/tawjihi-2009/islamic/term-1/${exam._id}`;
              const seoCardText = `امتحان دين توجيهي 2009 (الفصل الأول) — ${
