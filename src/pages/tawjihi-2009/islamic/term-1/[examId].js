...
export async function getServerSideProps({ params }) {
  try {
    ...
    if (!res.ok) {
      return {
        redirect: {
          destination: "/tawjihi-2009/islamic/term-1",
          permanent: true,
        },
      };
    }
...
إذا كنت تبحث عن "امتحانات دين توجيهي 2009 فصل أول" أو "بنك أسئلة تربية إسلامية توجيهي 2009"، فهذه الصفحة توضّح معلومات الامتحان، بينما التقديم الفعلي يتم من داخل حساب الطالب بعد تفعيل الاشتراك.`;
  ...
  useEffect(() => {
    ...
    // مميز للدين Term 1 حتى ما يصير تعارض
    const KEY = "__GE_FAQ_JSONLD_IS_T1__";
  ...
  return (
    <div className="bg-gray-900 text-white min-h-screen">
      <Head>
        ...
        {/* ✅ JSON-LD */}
        <script
          id="breadcrumb-jsonld-is-t1"
          key="breadcrumb-jsonld-is-t1"
          type="application/ld+json"
          dangerouslySetInnerHTML={{ __html: JSON.stringify(breadcrumbJsonLd) }}
        />
        <script
          id="article-jsonld-is-t1"
          key="article-jsonld-is-t1"
          type="application/ld+json"
