...
export async function getServerSideProps({ params }) {
  try {
    ...
    if (!res.ok) {
      return {
        redirect: {
          destination: "/tawjihi-2009/jordan-history/term-1",
          permanent: true,
        },
      };
    }
...
export default function JordanHistoryTerm1ExamSEO({ exam }) {
  ...
  const subjectShort = "تاريخ الأردن";
  ...
  const faqItems = [
    {
      q: "هل هذا الامتحان قريب من النمط الوزاري لتاريخ الأردن؟",
      a: "نعم، الامتحانات مصممة لتكون قريبة من النمط الوزاري وتساعدك على التدريب الواقعي قبل الامتحان.",
    },
    {
...
إذا كنت تبحث عن "امتحانات تاريخ الأردن توجيهي 2009 فصل أول" أو "بنك أسئلة تاريخ الأردن توجيهي 2009"، فهذه الصفحة توضّح معلومات الامتحان، بينما التقديم الفعلي يتم من داخل حساب الطالب بعد تفعيل الاشتراك.`;
      ...
      <main className="pt-24 pb-16 px-4 sm:px-6 max-w-4xl mx-auto" dir="rtl">
        ...
        <section className="mt-8 bg-gray-800/50 border border-yellow-500/10 rounded-2xl p-5 sm:p-6">
          <h2 className="text-base sm:text-lg font-extrabold text-yellow-300">
            امتحان {subjectLabel} توجيهي 2009 {termLabel} — تدريب وزاري تفاعلي
          </h2>
