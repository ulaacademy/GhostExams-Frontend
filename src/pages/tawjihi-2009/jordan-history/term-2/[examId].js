...
export default function JordanHistoryTerm2ExamSEO({ exam }) {
  const siteUrl = "https://ghostexams.com";
  const canonicalUrl = `${siteUrl}/tawjihi-2009/jordan-history/term-2/${exam._id}`;
  const ogImage = `${siteUrl}/og/jordan-history-2009.jpg`;

  const title = `${exam.examName} | تاريخ الأردن توجيهي 2009 الفصل الثاني - GhostExams`;
  const description = `معلومات امتحان تاريخ الأردن توجيهي 2009 (الفصل الثاني) — المدة ${exam.duration} دقيقة، عدد الأسئلة ${exam.questionsCount}. هذه صفحة معلومات فقط، وتقديم الامتحان يتم من داخل حساب الطالب بعد الاشتراك.`;

  const keywords = [
    exam.examName,
    "امتحانات تاريخ الأردن توجيهي 2009",
    "الفصل الثاني تاريخ الأردن 2009",
    ...
    <div className="bg-gray-900 text-white min-h-screen">
      ...
      <main className="pt-24 pb-14 px-4 max-w-4xl mx-auto" dir="rtl">
        <div className="flex items-center justify-between gap-3 mb-5">
          <Link
            href="/tawjihi-2009/jordan-history/term-2"
          ...
          <div className="text-[11px] sm:text-sm text-gray-300">
            تاريخ الأردن 2009 / الفصل الثاني / معلومات الامتحان
          </div>
        </div>
